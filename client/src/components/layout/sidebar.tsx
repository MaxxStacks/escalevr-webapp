...
// French translations for sidebar items
const translations = {
...
export default function Sidebar() {
  ...
  const getNavItems = () => {
    ...
    // Only admin can manage all users (including other admins)
    if (user?.role === "admin") {
      userItems.push(
    ...
    // Admin and service can see clients
    ...
    // Only admin can see service users
    if (user?.role === "admin") {
      userItems.push(
    ...
    // Admin and service can see technicians
    if (["admin", "service"].includes(user?.role as string)) {
      userItems.push(
    ...
    // Only admin can see claim agents
    if (user?.role === "admin") {
      userItems.push(
    ...
    // Settings item is accessible to everyone
    const settingsItem = [
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-primary/5">
        <div className="flex items-center">
          ...
          <Button 
            ...
            className="ml-auto text-muted-foreground hover:text-primary hover:bg-primary/10"
            title="Déconnexion"
          >
